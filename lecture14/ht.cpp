...
bool IntegerSetHTChain::insert(int data)
{
  ...
  // should search list and ensure no duplicates
  if ( !find(data) ) {
...
bool IntegerSetHTChain::find(int data) const
{
  list<int>& l = table[hash(data)];
...
{
  list<int>::iterator prev, it = table[hash(data)].begin();
  while(it != table[hash(data)].end()) {  
    if (*it==data) {
        table[hash(data)].erase(it);
        return true;
    }
    it++;
  }
...
int main()
{
  ...
  IntegerSet & set = *(new IntegerSetHTChain());
